...
const Channel = ({ language }) => {
    const [channelStats, setChannelStats] = useState({
        ...
        youtubeViews: 0,
        youtubeVideoCount: 0,
        channelUrl: 'https://l.wl.co/l?u=https%3A%2F%2Fyoutube.com%2F%40mahmouddesign_0%3Fsi%3D03AAh6tn5va_Inb3',
    });
    ...
    useEffect(() => {
        ...
        const fetchYouTubeStats = async () => {
            ...
            try {
                ...
                if (data.items && data.items.length > 0) {
                    ...
                    setChannelStats({
                        youtubeSubscribers: parseInt(channelData.statistics.subscriberCount, 10),
        ...
        const observerCallback = (entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting && !hasAnimated.current) {
                    ...
                    new CountUp(totalSubscribersRef.current, channelStats.youtubeSubscribers, countUpOptions).start();
                    new CountUp(videoCountRef.current, channelStats.youtubeVideoCount, countUpOptions).start();
                    new CountUp(viewsRef.current, channelStats.youtubeViews, countUpOptions).start();
        ...
        if (totalSubscribersRef.current) observer.observe(totalSubscribersRef.current);
    ...
    return (
        <div className='channel-sec'>
            <br/>
            <h2 className={`section-title ${language}`}>{translations[language].channel}</h2>
            <section className="channel">
                ...
                <div className={`youtube-stats ${language}`}>
                    ...
                    <p>
                        <strong>{translations[language].channelStats.subscribers}:</strong> {channelStats.youtubeSubscribers}
                    </p>
                    <p>
                        <strong>{translations[language].channelStats.totalViews}:</strong> {channelStats.youtubeViews}
                    </p>
                    <p>
